...
const { fileRemover, multerFileUploader, sendCodeToEmail, randomNumbersGenerator, nodeGmailAuth, nodemailerTransporter, dateStatementReturn, isValueNotNull, paginationFiltrationWeb } = require('../../services/utils');
...
module.exports = {
    ...
    async getAdminsByFilterWeb(req, res, next) {
        try {
            ...
            const adminsCount = await Admin.count({ where: myWhere, });
            const pageFilterResult = paginationFiltrationWeb(req.query, adminsCount, myWhere);
            const filteredAdmins = await Admin.findAll(pageFilterResult.filter);

            return res.status(http_status_codes.StatusCodes.OK).json({
                statusCode: http_status_codes.StatusCodes.OK,
                isSuccess: true,
                message: "Admins by filter for web data fetched successfully!",
            ...
            return res.status(http_status_codes.StatusCodes.INTERNAL_SERVER_ERROR).json({
                statusCode: http_status_codes.StatusCodes.INTERNAL_SERVER_ERROR,
                isSuccess: false,
                message: "Error occurred in getting Admins by filter for web!",
                error: error,
            });
