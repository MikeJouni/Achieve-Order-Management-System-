...
const { fileRemover, multerFileUploader, multerFileUploaderWithoutCompress, isValueNotNull, paginationFiltrationWeb } = require('../../services/utils');
...
module.exports = {
    ...
    async getCustomVariablesByFilterWeb(req, res, next) {
        try {
            ...
            const customVariablesCount = await CustomVariable.count({ where: myWhere, });
            const pageFilterResult = paginationFiltrationWeb(req.query, customVariablesCount, myWhere);
            const filteredCustomVariables = await CustomVariable.findAll(pageFilterResult.filter);

            return res.status(http_status_codes.StatusCodes.OK).json({
                statusCode: http_status_codes.StatusCodes.OK,
                isSuccess: true,
                message: "CustomVariables by filter for web data fetched successfully!",
                customVariables: filteredCustomVariables,
            ...
            return res.status(http_status_codes.StatusCodes.INTERNAL_SERVER_ERROR).json({
                statusCode: http_status_codes.StatusCodes.INTERNAL_SERVER_ERROR,
                isSuccess: false,
                message: "Error occurred in getting CustomVariables by filter for web!",
                error: error,
            });
