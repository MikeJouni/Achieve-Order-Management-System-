...
const { randomNumbersGenerator, dateStatementReturn, isValueNotNull, paginationFiltrationWeb } = require("../../services/utils.js");
...
module.exports = {
    ...
    async getByCompanyDateAndFilter(req, res, next) {
        try {
            ...
            return res.status(http_status_codes.StatusCodes.OK).json({
                statusCode: http_status_codes.StatusCodes.OK,
                isSuccess: true,
                message: "Orders by Company, Date and filter data fetched successfully!",
                orders: myOrders,
            });
            ...
            return res.status(http_status_codes.StatusCodes.INTERNAL_SERVER_ERROR).json({
                statusCode: http_status_codes.StatusCodes.INTERNAL_SERVER_ERROR,
                isSuccess: false,
                message: "Error occurred in getting Orders by Company, Date and filter!",
                error: error,
            });
    ...
    async getOrdersByFilterWeb(req, res, next) {
        try {
            ...
            const ordersCount = await Order.count({ where: myWhere, });
            const pageFilterResult = paginationFiltrationWeb(req.query, ordersCount, myWhere, [], ['orderItemsDetail']);
            const filteredOrders = await Order.findAll(pageFilterResult.filter);

            return res.status(http_status_codes.StatusCodes.OK).json({
                statusCode: http_status_codes.StatusCodes.OK,
                isSuccess: true,
                message: "Orders by filter for web data fetched successfully!",
            ...
            return res.status(http_status_codes.StatusCodes.INTERNAL_SERVER_ERROR).json({
                statusCode: http_status_codes.StatusCodes.INTERNAL_SERVER_ERROR,
                isSuccess: false,
                message: "Error occurred in getting Orders by filter for web!",
                error: error,
            });
