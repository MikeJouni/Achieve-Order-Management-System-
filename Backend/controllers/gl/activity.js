...
module.exports = {
    ...
    async getActivitiesByFilterWeb(req, res, next) {
        try {
            ...
            const activitiesCount = await Activity.count({ where: myWhere, });
            const pageFilterResult = paginationFiltrationWeb(req.query, activitiesCount, myWhere);
            const filteredActivities = await Activity.findAll(pageFilterResult.filter);

            return res.status(http_status_codes.StatusCodes.OK).json({
                statusCode: http_status_codes.StatusCodes.OK,
                isSuccess: true,
                message: "Activities by filter for web data fetched successfully!",
            ...
            return res.status(http_status_codes.StatusCodes.INTERNAL_SERVER_ERROR).json({
                statusCode: http_status_codes.StatusCodes.INTERNAL_SERVER_ERROR,
                isSuccess: false,
                message: "Error occurred in getting Activities by filter for web!",
                error: error,
            });
