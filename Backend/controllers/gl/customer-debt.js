...
module.exports = {
    ...
    async getCustomerDebtsByFilterWeb(req, res, next) {
        try {
            ...
            const customerDebtsCount = await CustomerDebt.count({ where: myWhere, });
            const pageFilterResult = paginationFiltrationWeb(req.query, customerDebtsCount, myWhere);
            const filteredCustomerDebts = await CustomerDebt.findAll(pageFilterResult.filter);

            return res.status(http_status_codes.StatusCodes.OK).json({
                statusCode: http_status_codes.StatusCodes.OK,
                isSuccess: true,
                message: "CustomerDebts by filter for web data fetched successfully!",
                customerDebts: filteredCustomerDebts,
            ...
            return res.status(http_status_codes.StatusCodes.INTERNAL_SERVER_ERROR).json({
                statusCode: http_status_codes.StatusCodes.INTERNAL_SERVER_ERROR,
                isSuccess: false,
                message: "Error occurred in getting CustomerDebts by filter for web!",
                error: error,
            });
