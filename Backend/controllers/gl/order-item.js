...
module.exports = {
    ...
    async getOrderItemsByFilterWeb(req, res, next) {
        try {
            ...
            const orderItemsCount = await OrderItem.count({ where: myWhere, });
            const pageFilterResult = paginationFiltrationWeb(req.query, orderItemsCount, myWhere);
            const filteredOrderItems = await OrderItem.findAll(pageFilterResult.filter);

            return res.status(http_status_codes.StatusCodes.OK).json({
                statusCode: http_status_codes.StatusCodes.OK,
                isSuccess: true,
                message: "OrderItems by filter for web data fetched successfully!",
            ...
            return res.status(http_status_codes.StatusCodes.INTERNAL_SERVER_ERROR).json({
                statusCode: http_status_codes.StatusCodes.INTERNAL_SERVER_ERROR,
                isSuccess: false,
                message: "Error occurred in getting OrderItems by filter for web!",
                error: error,
            });
