...
module.exports = {
    ...
    async getNoteRemindersByFilterWeb(req, res, next) {
        try {
            ...
            const noteRemindersCount = await NoteReminder.count({ where: myWhere, });
            const pageFilterResult = paginationFiltrationWeb(req.query, noteRemindersCount, myWhere);
            const filteredNoteReminders = await NoteReminder.findAll(pageFilterResult.filter);

            return res.status(http_status_codes.StatusCodes.OK).json({
                statusCode: http_status_codes.StatusCodes.OK,
                isSuccess: true,
                message: "NoteReminders by filter for web data fetched successfully!",
                noteReminders: filteredNoteReminders,
            ...
            return res.status(http_status_codes.StatusCodes.INTERNAL_SERVER_ERROR).json({
                statusCode: http_status_codes.StatusCodes.INTERNAL_SERVER_ERROR,
                isSuccess: false,
                message: "Error occurred in getting NoteReminders by filter for web!",
                error: error,
            });
